#!/usr/bin/env python
"""
Power-draw scatter plot for the shortlisted object and pose models.

Run:
  python Figures/scatter/best_model_analysis_object_pose_power.py
"""
...
def _load_csv() -> pd.DataFrame:
    ...
    required = ["Model", "Location", MAP_COLUMN, X_COLUMN]
...
def _prepare_subset(df: pd.DataFrame, *, mask: pd.Series, min_map: float) -> pd.DataFrame:
    ...
    subset = subset.dropna(subset=[MAP_COLUMN, X_COLUMN]).copy()
...
def _format_label(model_name: str) -> str:
    label = str(model_name).replace(".engine", "").replace(".pt", "")
...
def _apply_annotations(ax: object, rows: pd.DataFrame, offsets: dict[str, tuple[int, int]], color: str) -> None:
    ...
    for _, row in rows.iterrows():
        ...
        annotation = ax.annotate(
            ...
            xy=(row[X_COLUMN], row[MAP_COLUMN]),
            xytext=(dx, dy),
...
def main() -> int:
    ...
    legend_handles = [
        Line2D([0], [0], marker="o", color="none", markerfacecolor=BASE_POINT_COLOR, markersize=4.5, label="Other models"),
        Line2D(
            [0],
            [0],
            marker="o",
    ...
    fig.supxlabel(X_LABEL, fontsize=8, y=0.10)
    fig.tight_layout(pad=0.1)
    fig.subplots_adjust(left=0.08, right=0.995, bottom=0.22, top=0.94, wspace=0.16)
    plt.show()
    prompt_save_figure(fig, default_name="best_model_analysis_object_pose_power")
    return 0
