#!/usr/bin/env python
"""
Line plot of mAP50-95 vs FPS for 11x DS3 baseline vs pruned engines.

Run:
  python Figures/line_map50_95_vs_fps_11_ds3_pruning.py
"""
...
def main() -> int:
    ...
    df = df[df["kind"].isin(["baseline", "pruned"])].copy()
    if df.empty:
        print("Error: no baseline/pruned DS3 engine rows found.", file=sys.stderr)
        return 1
    ...
    pruned = df[df["kind"] == "pruned"].copy()
    for pct in sorted(pruned["prune_pct"].dropna().unique()):
        group = pruned[pruned["prune_pct"] == pct].copy()
        if not group.empty:
            lines.append((f"p{int(pct)}", group))
    ...
    ax.set_title(f"{MAP_COLUMN} vs FPS (11x DS3 Baseline and Pruned)")
    ax.grid(True, linestyle="--", alpha=0.4)
    ax.legend(frameon=False, fontsize=8)

    fig.tight_layout()
    plt.show()
    prompt_save_figure(fig, default_name="map50_95_vs_fps_11_ds3_pruning")
    return 0
