#!/usr/bin/env python
"""
Line plot of mAP50-95 vs FPS for 11x DS3 baseline vs quantized engines.

Run:
  python Figures/line_map50_95_vs_fps_11_ds3_quant.py
"""
...
def main() -> int:
    ...
    df = df[df["kind"].isin(["baseline", "quant"])].copy()
    if df.empty:
        print("Error: no baseline/quantized DS3 engine rows found.", file=sys.stderr)
        return 1
    ...
    quant = df[df["kind"] == "quant"].copy()
    for q in ["fp16", "int8"]:
        group = quant[quant["quant_level"] == q].copy()
        if not group.empty:
            lines.append((q, group))
    ...
    ax.set_title(f"{MAP_COLUMN} vs FPS (11x DS3 Baseline and Quantized)")
    ax.grid(True, linestyle="--", alpha=0.4)
    ax.legend(frameon=False, fontsize=8)

    fig.tight_layout()
    plt.show()
    prompt_save_figure(fig, default_name="map50_95_vs_fps_11_ds3_quant")
    return 0
