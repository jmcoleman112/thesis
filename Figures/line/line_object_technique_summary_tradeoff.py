#!/usr/bin/env python
"""
Compact mAP50-95 vs latency plot for the object technique summary.

The table in Thesis/tables/object_technique_summary.tex remains percentage-based.
This companion plot keeps the absolute uncompressed and accelerated anchors.
Input reduction and quantisation are projected from family-matched baseline
.engine deltas, while pruning and distillation are projected from family-matched
baseline .pt deltas, back into the same absolute mAP50-95 vs latency space using:
  research/model_summaries.csv

Run:
  python Figures/line/line_object_technique_summary_tradeoff.py
  python Figures/line/line_object_technique_summary_tradeoff.py --no-show
  python Figures/line/line_object_technique_summary_tradeoff.py --output Figures/produced_images/object_technique_summary_tradeoff.png
"""
...
DEFAULT_OUTPUT = (
    Path(__file__).resolve().parents[2] / "Figures" / "produced_images" / "object_technique_summary_tradeoff.png"
)
MAP_COLUMN = "mAP50-95"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot average object technique mAP50-95 vs latency.")
...
def main() -> int:
    ...
    try:
        if args.output is not None:
            ...
            plt.show()
            saved_path = prompt_save_figure(fig, default_name="object_technique_summary_tradeoff")
