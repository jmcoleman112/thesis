...
Before individual techniques are examined in detail, it is useful to first take a higher-level view of the overall accuracy--efficiency trade-offs across the main compression strategies to see which approaches are most promising for real-time embedded soccer analysis. This will be broken into Object Detection and Pose Estimation subsections, since the two tasks show different patterns of sensitivity to the various techniques. 


Table \ref{tab:object_technique_summary} below displays the average change in mAP50-95 and latency for each of the main compression techniques applied to the YOLO11 and YOLO26 object detection models, averaged across the paired N, S, M, and L variants.

\input{tables/object_technique_summary}

The object-detection summary shows a clear ordering in the average trade-offs. Hardware acceleration is the safest first optimisation step, reducing latency by roughly one third while preserving almost identical mAP$_{50\text{--}95}$. Quantisation produces the strongest speed-up overall, with the largest gains coming from INT8 deployment, but it also introduces the most visible accuracy penalty among the single-step methods. FP16 is especially notable here, since it captures most of the latency benefit of quantisation while remaining much closer to the baseline in mAP$_{50\text{--}95}$. Pruning sits in the middle ground, offering moderate speed improvements with a smaller average accuracy cost than INT8, while knowledge distillation changes latency very little and is therefore better understood as an accuracy-supporting technique than as a direct runtime optimisation. 

The combination of these various techniques can now be explored, as shown in Table \ref{tab:object_combo_summary}. 


\input{tables/object_combo_summary}

The object-model combinations reinforce the same overall pattern, but make the trade-off boundary more explicit. The most aggressive paths, particularly those ending in INT8 after pruning, achieve the largest average latency reductions, but they do so alongside the largest losses in mAP$_{50\text{--}95}$. By contrast, combinations that retain FP16 precision or pair acceleration with distillation are more balanced, preserving accuracy more effectively while still delivering a substantial runtime improvement. This suggests that for object detection, the strongest deployment candidates are not necessarily the most compressed models, but the intermediate configurations that combine acceleration with moderate compression and some form of accuracy preservation.

\noindent Table \ref{tab:pose_technique_summary} applies the same summary view to the pitch keypoint models, averaging the effect of each base technique across the paired YOLO11-pose and YOLO26-pose N, S, M, and L variants.

\input{tables/pose_technique_summary}

Compared with object detection, the pose results are much less forgiving. Hardware acceleration again provides a strong and low-risk latency improvement, and quantisation still delivers the largest speed-up, but both effects are now accompanied by a more fragile accuracy profile. The clearest contrast is pruning: whereas the object models tolerated parameter removal reasonably well on average, the pose models lose a very large proportion of mAP$_{50\text{--}95}$ for only modest latency gains. Distillation remains close to latency-neutral here as well, but even its average accuracy cost is still much smaller than the pruning penalty, which makes it a comparatively safer way to support compact pose models.

\noindent Table \ref{tab:pose_combo_summary} extends this comparison to the combined pose-model compression paths.

\input{tables/pose_combo_summary}

The combined pose results confirm that aggressive multi-step compression is much harder to justify for keypoint localisation than for object detection. INT8-based combined paths and pruning-heavy routes produce large runtime improvements, but the associated mAP$_{50\text{--}95}$ losses are severe enough that many of these models would be difficult to use in a reliable homography pipeline. In contrast, the more balanced combined configurations, particularly those involving acceleration or FP16 alongside distillation and lighter pruning, retain substantially more analytical performance while still improving latency. Taken together, these two pose summary tables indicate that the pitch keypoint task has a much narrower safe compression range than the object task, and that deployment choices for the pose stage must therefore be made more conservatively.

These table-level summaries provide the broad trends across the full model inventory. The following figures then examine each compression technique in more detail, showing how these average effects emerge across model scale and transformation path.
