...
Recent YOLO detectors, such as the Ultralytics YOLO11 and YOLO26 families, generally follow a backbone-neck-head design \cite{UltralyticsYOLO}. In this, the backbone extracts features from the image at increasing levels of abstraction. The neck then combines these various features so that both fine spatial detail and higher-level semantic information can be used by the detection head. To do so, the backbone utilises a variety of convolutional layers (Conv) and variations ofCSP bottlenecks with 3 convolutions (C3K2). The neck uses upsampling, concatentation, and C3K2 blocks, allowing the head to produce predictions at three resolutions, corresponding broadly to small, medium, and large objects \cite{UltralyticsYOLO}. A simplified version of this architecture is displayed below in Figure \ref{fig:yolo11_architecture}.
