...
The reported features and statistics of some implemented software solutions are displayed below in Table \ref{softwarestacks}. Note that reported FPS values are taken from original implementations, and are not directly comparable across hardware. Also note that model accuracy and size values ($\text{mAP}_{50}$, FLOPS, Parameters) cannot be explored here, due to significant differences in software structures (both in amount of models, model output information, and implementation output). Instead, the accuracy and size of the models used in these will be explored explicitly in subsections \textbf{Object Detection} and \textbf{Key-point Detection}.
